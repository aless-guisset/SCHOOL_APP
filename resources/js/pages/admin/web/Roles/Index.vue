...
defineProps<{
    roles: {
        data: Array<{ id: number; name: string; reference: string | null; is_active: boolean }>;
...
<template>
    ...
    <AppLayout>
        <div class="p-4 md:p-6">
            <FlashMessage />
            <PageHeader :title="t('nav.roles')" :description="`${roles.total} rôles`">
                <template #actions>
                    <Button as-child size="sm">
                        <Link href="/roles/create"><Plus class="size-4" />{{ t('action.create') }}</Link>
                    </Button>
                </template>
            </PageHeader>
            <DataTable :data="roles" :columns="columns" :row-href="(row) => `/roles/${row.id}`" empty-message="Aucun rôle." />
        </div>
    </AppLayout>
</template>
