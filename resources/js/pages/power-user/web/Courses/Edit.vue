...
const props = defineProps<{
    course: { id: number; name: string; reference: string | null; description: string | null };
}>();

const form = useForm({
    name: props.course.name,
    reference: props.course.reference ?? '',
    description: props.course.description ?? '',
});

const breadcrumbs = [
    { label: t('nav.courses'), href: '/courses' },
    { label: props.course.name, href: `/courses/${props.course.id}` },
    { label: t('action.edit') },
];
</script>
...
<template>
    ...
    <AppLayout>
        <div class="p-4 md:p-6 max-w-xl">
            <FlashMessage />
            <PageHeader :title="`Modifier : ${course.name}`" :breadcrumbs="breadcrumbs" />
            <Card>
                <CardContent class="pt-6">
                    <form class="space-y-4" @submit.prevent="form.patch(`/courses/${course.id}`)">
                        ...
                        <div class="flex gap-3 pt-2">
                            ...
                            <Button variant="outline" as-child>
                                <Link :href="`/courses/${course.id}`">{{ t('action.cancel') }}</Link>
                            </Button>
                        </div>
                    </form>
                </CardContent>
