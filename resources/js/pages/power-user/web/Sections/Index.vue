...
defineProps<{
    sections: {
        data: Array<{ id: number; name: string; reference: string | null; description: string | null; is_active: boolean }>;
...
<template>
    ...
    <AppLayout>
        <div class="p-4 md:p-6">
            <FlashMessage />
            <PageHeader :title="t('nav.sections')" :description="`${sections.total} section(s)`">
                <template #actions>
                    <Button as-child size="sm">
                        <Link href="/sections/create"><Plus class="size-4" />{{ t('action.create') }}</Link>
                    </Button>
                </template>
            </PageHeader>
            <DataTable :data="sections" :columns="columns" :row-href="(row) => `/sections/${row.id}`" empty-message="Aucune section." />
        </div>
    </AppLayout>
</template>
